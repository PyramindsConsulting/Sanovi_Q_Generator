...
$data_2s_3s=adding2s_3s($qty_2s,$qty_3s);
...
$pdf->Cell(35,5,'Price in '.$currency,0,0);
...
$pdf->Cell(35,5,$lht_data["discountPercentageOnLicense"],0,0,'R');
$pdf->Cell(36,5,'-',0,0,'R');
$finalLicenseCost=currency_format($lht_data["finalLicenseCost"],$currency);
$pdf->Cell(38,5,currency_format($lht_data["finalLicenseCost"],$currency),0,0,'R');
$pdf->ln();
$pdf->Cell(60,5,'- Product Support',0,0);
$pdf->Cell(16,5,currency_format($lht_data["productSupportCost"],$currency),0,0,'R');
$pdf->Cell(35,5,$lht_data["discountPercentageOnSupport"],0,0,'R');
$pdf->Cell(36,5,'-',0,0,'R');
$pdf->Cell(38,5,currency_format($lht_data["finalSupportCost"],$currency),0,0,'R');
$pdf->ln();
$pdf->Cell(60,5,'- Professional Services',0,0);
$pdf->Cell(16,5,currency_format($lht_data["PSCost"],$currency),0,0,'R');
$pdf->Cell(35,5,$lht_data["discountPercentageOnPS"],0,0,'R');
$pdf->Cell(36,5,'-',0,0,'R');
$pdf->Cell(38,5,currency_format($lht_data["finalPSCost"],$currency),0,0,'R');
$pdf->ln();
$pdf->Cell(60,5,'- DRM Product Training',0,0);
$pdf->Cell(16,5,$lht_data["trainingCost"],0,0,'R');
$pdf->Cell(35,5,$lht_data["discountPercentageOnTraining"],0,0,'R');
$pdf->Cell(36,5,'-',0,0,'R');
$pdf->Cell(38,5,$lht_data["finalTrainingCost"],0,0,'R');
$pdf->ln();
$pdf->ln();
$pdf->SetFont('Arial','B',10);
  ...
  for($i=0;$i<$qty_and_questions_count;$i++) {
    ...
    $space_left=$page_height-($pdf->GetY()+$bottom_margin); // space left on page
      if ($i/1==floor($i/1) && $height_of_cell > $space_left) {
        ...
        $x=$pdf->GetX();
        $y=$pdf->GetY();
        $pdf->MultiCell(40,5,'',0,'L');
        $pdf->SetXY($x+40,$y);
        $x=$pdf->GetX();
        $y=$pdf->GetY();
        $pdf->MultiCell(80,5,get_question($qty_and_questions_2s[$i][0]),0,'L');
        $pdf->SetXY($x+120,$y);
        $x=$pdf->GetX();
        $y=$pdf->GetY();
  ...
  for($i=0;$i<$count_qty_and_questions_3s;$i++) {
    ...
    $space_left=$page_height-($pdf->GetY()+$bottom_margin); // space left on page
      if ($i/1==floor($i/1) && $height_of_cell > $space_left) {
        ...
        $x=$pdf->GetX();
        $y=$pdf->GetY();
        $pdf->MultiCell(80,5,get_question($qty_and_questions_3s[$i][0]),0,'L');
        $pdf->SetXY($x+120,$y);
        $x=$pdf->GetX();
        $y=$pdf->GetY();
...
$pdf->Cell(20,5,ucfirst($prof_services[0][1]),0,0,'L');

if(ucfirst($prof_services[1][1])=='no'){
for($i=0;$i<15;$i++){
    $pdf->SetFont('Arial','',8);
    $pdf->Cell(40,5,'',0,0,'L');
...
//save the pdf file in local disc
$filename="sanovi_quote.pdf";
